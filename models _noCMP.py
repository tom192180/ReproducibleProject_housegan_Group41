...
class CMP(nn.Module):
    ...
    def forward(self, feats, edges=None):
        ...
        pooled_v_pos = torch.zeros(V, feats.shape[-3], feats.shape[-1], feats.shape[-1], dtype=dtype, device=device)# preparing seond and third group of layers
        pooled_v_neg = torch.zeros(V, feats.shape[-3], feats.shape[-1], feats.shape[-1], dtype=dtype, device=device)
        
        # pool positive edges
        pos_inds = torch.where(edges[:, 1] > 0) # finding positive related room index
        pos_v_src = torch.cat([edges[pos_inds[0], 0], edges[pos_inds[0], 2]]).long() # all room numbers that are adjacent
        pos_v_dst = torch.cat([edges[pos_inds[0], 2], edges[pos_inds[0], 0]]).long() # all room numbers that are adjacent
        pos_vecs_src = feats[pos_v_src.contiguous()]    # extracting features using room index obtained from above 
        pos_v_dst = pos_v_dst.view(-1, 1, 1, 1).expand_as(pos_vecs_src).to(device) # reshape in to four dimension
        pooled_v_pos = pooled_v_pos.scatter_add(0, pos_v_dst, pos_vecs_src) # add the adjacent rooms features together regarding to sepecifc room Rm  x 16 x m x m
...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.init_size = 32 // 4
        self.l1 = nn.Sequential(nn.Linear(138, 16 * self.init_size ** 2)) # from n x138 to n x (16 x 8 x 8)
        self.upsample_1 = nn.Sequential(*conv_block(16, 16, 4, 2, 1, act="leaky", upsample=True)) # to n x16 x 16 x 16
        self.upsample_2 = nn.Sequential(*conv_block(16, 16, 4, 2, 1, act="leaky", upsample=True)) # to n x 16 x 32 x 32
    ...
    def forward(self, z, given_y=None, given_w=None): # in batch on, input z (268 x 128) y(268x10) w (1058x3)
        z = z.view(-1, 128)
        # include nodes
        if True:
            y = given_y.view(-1, 10)
            z = torch.cat([z, y], 1) # n x 138
        x = self.l1(z)  # n x 1024    
        x = x.view(-1, 16, self.init_size, self.init_size) # n x 16 x 8 x 8
        x = self.cmp_1(x, given_w).view(-1, *x.shape[1:]) #  n x 16 
        x = self.upsample_1(x)
...
class Discriminator(nn.Module):
    ...
    def forward(self, x, given_y=None, given_w=None, nd_to_sample=None):
        ...
        x = x.view(-1, 1, 32, 32) # n x 1 x 32 x 32
        # include nodes
        if True:
            #y = self.l1(given_y) # expand room time from 10d to n x 8192
            #y = y.view(-1, 8, 32, 32) # reshape to 8x32 x 32
            #x = torch.cat([x, y], 1)
        x = self.encoder(x)
